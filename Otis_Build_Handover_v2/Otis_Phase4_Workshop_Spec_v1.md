...
Members have already (Phase 3): read the report, reflected on the focus item with Otis (telling stories, generating behaviors), typed their own NEVER/ALWAYS/SOMETIMES entries into their board (min 2 each for Never and Always), and pressed Finish. The consultant has reviewed the pooled behaviors and released the approved set to `workshop_seed`.

**The behaviors in the workshop pool come from `workshop_seed`** — member-written, coached for observability and anonymity by Otis during Phase 3, and reviewed by the consultant. The workshop does not re-run any analysis; it consumes what's already there.

Facilitator opens the session; members enter the workshop room from their profiles.

**Team size note:** max supported 12. Otis computes the structure from team size and tells the facilitator what to run (see §8).

---

## 3. M1 — ORIENT (~6 min)

**Members' shared view:** the focus frame only — the PS item, a summary of the situations members described (from the deferred context tags: "your team discussed this in the context of meetings, chats, and handovers"), and one line of why ("this is where your team's scores suggest there's room to grow"). No individual scores, no individual data.
...
**The pooled board:** all members' submitted behaviors (from Phase 3, approved by the consultant), colour-coded by member, with toggles to show/hide individuals. The member's own and their partner's are highlighted; everyone else dimmed by default.

> "Toggle to just you and [partner]. Compare what you each generated — where you overlap and where you differ."

Near-duplicate entries are collapsed visually (show the clearest phrasing once, not both versions).
...
A structured form, not a blank page. Pairs select their chosen behaviours and type the observability line:
...
This is the same observability discipline as Phase 3's live coaching, applied again at the point of writing the observability line. Nudge at most twice, then accept.
