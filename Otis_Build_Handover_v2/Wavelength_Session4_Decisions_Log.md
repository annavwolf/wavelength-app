...
## Pivot decisions (added this session — supersede earlier where noted)

**D-043 — RETIRED.** Individual item-selection for the interview no longer exists. Phase 1 is survey-only; item selection happens only at the team level (D-051), and the selected item drives Phase 3's reflection+generation activity, not a per-member interview.

**D-046 — REFRAMED as live coaching.** The multi-member and anonymity concern is now handled by Otis coaching members LIVE during Phase 3 behaviour entry (not a post-hoc coding filter). If a member names someone or types an identifying comment, Otis nudges once gently ("let's keep this about the team"). If the member insists, the entry is flagged for the consultant review step. The old `sensitive_specific` field from the superseded Coding Redesign doc is replaced by this live-flag approach.

**D-047 — Fallback reused.** The stretch-case for high-scoring teams still defaults to Innovate items 9 or 11, consistent with the original design. Applied now at team level, not individual.

**Pivot: Phase 1 is survey-only.** The 4-bucket interview loop (Situation→Behavior→Outcome→Reflection) is removed from Phase 1 entirely. Phase 1 collects: PS education, the 12-item survey, purpose, coordination ratings. That's it.

**Pivot: Phase 3 is the merged reflection + generation step.** Stories and behaviour generation happen in Phase 3, focused on the team-selected item (D-051). Every member reflects on the SAME item, enabling cross-member convergence by design.

**Pivot: consultant review is a team-level gate.** The consultant reviews the pooled member behaviors AFTER all members finish Phase 3 and BEFORE the workshop is scheduled. The member's "we'll be in touch" closing goes out before review. The consultant can edit entries; the member won't see their original changed.

**Pivot: situation-tagging is deferred and light.** Story text is saved raw. A lightweight batch LLM pass after Phase 3 submission tags each story with a context category (meeting, chat, email, etc.). No full coding, no gerund extraction, no clustering of stories.

**Pivot: no more full coding/clustering pipeline for stories.** The Coding Spec, clustering.ts (for stories), and the Coding & Clustering Redesign are all superseded. Clustering MAY still be used for lightweight dedup/convergence-sizing of member-submitted behaviors in the Phase 4 pool, but that's a simpler pass, not the 5-stream coding pipeline.

**Pivot: 12 action-phrase pairs locked.** Each PS item has a paired action phrase for use in the bridge script (Phase 3 §3.5). Item 3 inverted for reverse-scoring.

**Pivot: Otis-as-assistant via OtisChatBubble.** During Phase 3's activity, Otis is available as a chat helper but never writes directly to the member's board. The member types their own entries. This reuses the existing component and avoids new tool-wiring.
