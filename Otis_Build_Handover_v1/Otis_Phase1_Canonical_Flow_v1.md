...
Replaces whatever is currently seeded in `ps_statements`. Final wording, per the canonical source material (now captured in the Phase 2 Coding Spec):
...
> Light concreteness nudge: if the answer is only a feeling ("it was frustrating"), Otis may ask once for a concrete consequence: *"Did that change anything about how the work went, or how you acted afterwards?"* Feelings are valid outcomes and should be kept — this nudge seeks an *additional* concrete effect, it does not replace or dismiss the emotional one.
...
>
> **OBSERVABILITY PUSH (required).** If the reflection is vague, aspirational, or trait-based — "people should listen," "be more respectful," "communicate better" — Otis must reflect it back and ask what it would concretely look like:
> *"What would that actually look like in the moment — what would someone do or say?"*
> Target: turn "people should listen" into an observable action such as "let each person finish before responding."
> **Why this matters:** the IN-behaviors captured here become the candidate ALWAYS behaviors in the team workshop, and eventually the team's written agreement. A vague IN-behavior produces an unenforceable agreement. Observability matters MORE on the IN side than the OUT side, because the IN behaviors are what the team commits to.
> Apply the same test as Turn 2: could someone watching the team see or hear this happen? If not, push once more. Do not push more than twice — accept what the member gives on the third attempt rather than badgering.
...
Decided by working backward from what Phase 2 coding (per the canonical source material (now captured in the Phase 2 Coding Spec)) needs as input: data already split into the four buckets, already linked to the source PS item, so Phase 2 doesn't have to re-parse a conversation blob.
...
1. **RESOLVED — Phase 2 pipeline.** The older v3.1 eight-stage fish/STARR pipeline (D-038) is superseded. The canonical Phase 2 design is now the **Phase 2 Coding Spec** (free text → labels) followed by the **Phase 2 Analytics Spec** (clustering → aggregation → interpretation → dashboard). D-048 records which earlier safeguards carry forward (D-030, D-031, D-033, D-036, D-037, D-039, D-040, D-041) — see the Carried-Forward Decisions Reference.
2. **RESOLVED — dashboard structure.** The dashboard is **one page with two tabs**: "Analytics & Insights" and "Workshop", with Otis present as a persistent chat bubble in both. The existing three-zone structure is re-anchored and nested into the Analytics tab rather than replaced. Full layout in the **Phase 2 Analytics Spec §4**.
