...
export async function POST(request: NextRequest) {
  ...
  const [{ data: member, error: memberError }, { data: acknowledgement, error: acknowledgementError }] = await Promise.all([
    ...
    supabaseAdmin.from("member_privacy_acknowledgements").select("member_id, privacy_notice_version").eq("member_id", memberId).maybeSingle(),
  ]);
  ...
  const [statementsRes, responsesRes, purposeRes, rosterRes, coordinationRes] = await Promise.all([
    ...
    supabaseAdmin.from("ps_responses").select("id", { count: "exact", head: true }).eq("member_id", memberId).eq("round", 1),
    supabaseAdmin.from("purpose_responses").select("id", { count: "exact", head: true }).eq("member_id", memberId),
    supabaseAdmin.from("members").select("member_id", { count: "exact", head: true }).eq("team_id", member.team_id),
    supabaseAdmin.from("coordination_ratings").select("id", { count: "exact", head: true }).eq("member_id", memberId),
  ]);
