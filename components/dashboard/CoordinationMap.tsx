...
import { FAV_GREEN, NEU_YELLOW, type CoordinationPair, type AsymmetricPair } from "./types";
...
export default function CoordinationMap({
  ...
  const freqColor: Record<string, string> = { daily: FAV_GREEN, weekly: "#3B82F6", occasionally: "#9CA3AF", rarely: "#D1D5DB" };
      ...
      {pairs.filter((p) => p.to_private_code).map((p, i) => {
        ...
        return (
          <line key={i} x1={f.x} y1={f.y} x2={t.x} y2={t.y}
            stroke={isAsym ? NEU_YELLOW : (freqColor[p.frequency] ?? "#D1D5DB")}
