...
2. Apply `supabase/migrations/0023_beta_privacy_and_retention.sql` through `supabase/migrations/0033_phase3_resume_checkpoint.sql` in numeric order. Use the current repository copy of `0024`: it uses `array_agg`, not the unsupported `min(uuid)` from an earlier draft. Migration `0033` stores the exact Phase 3 page and furthest page reached so participants resume reliably across devices.
...
5. Configure and verify `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `OTIS_APP_URL`, `MEMBER_SESSION_SECRET`, `MEMBER_LOGIN_RATE_LIMIT_SECRET`, `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY`, `RESEND_API_KEY`, a verified `RESEND_FROM_EMAIL` domain, and `EARLY_ACCESS_CODE_HASHES`. `OTIS_APP_URL` must be the deployed Otis application URL (for example its Vercel URL or a dedicated app subdomain), never `https://wavelength.team` or `https://www.wavelength.team`, which are the marketing site. Keep `NEXT_PUBLIC_APP_URL` equal to it only if client code requires it; the server uses it only as a transition fallback. `SUPABASE_SERVICE_ROLE_KEY` is server-only and required after the RLS migration; a missing Resend configuration prevents actual invite and magic-link delivery. `MEMBER_LOGIN_RATE_LIMIT_SECRET` is a dedicated long HMAC secret for the durable magic-link limiter and safely falls back to `MEMBER_SESSION_SECRET` during transition. `EARLY_ACCESS_CODE_HASHES` must contain SHA-256 hashes only, never raw codes or a `NEXT_PUBLIC_*` value. Also set a distinct long `EARLY_ACCESS_PENDING_COOKIE_SECRET` if possible; the app safely falls back to `MEMBER_SESSION_SECRET` during transition.
6. If you are enabling enhanced audio, create a Deepgram project API key and set server-only `DEEPGRAM_API_KEY` in Vercel. `OTIS_DEEPGRAM_TTS_MODEL` and `OTIS_DEEPGRAM_STT_MODEL` are optional server-only model overrides. Where Deepgram offers permission choices, grant only the speech-to-text and text-to-speech features Otis needs. Do not set an audio key in any `NEXT_PUBLIC_*` variable. In the Deepgram project's billing settings, turn **Auto-Load** off before enabling the key; keep it off during the pilot and review usage and remaining credit regularly. Migration `0031_pilot_audio_quota.sql` adds Otis's durable shared monthly pilot cap (by default, 1,000,000 TTS characters and 500 STT minutes / 512 MiB); adjust only with the bounded server-only `OTIS_AUDIO_PILOT_*` variables if you deliberately need a lower limit. Add Vercel WAF rate-limit rules for `/api/audio/*`, `/api/early-access`, and `/api/early-access/pending` before exposing the app publicly.
7. Have the privacy owner complete the unresolved factual details listed in `Otis_Beta_Participant_Privacy_Notice_v0.5.md` before public beta. Deploying v0.5 deliberately requires every participant who acknowledged an earlier notice to acknowledge the updated notice once, because the optional enhanced-audio processor changed to Deepgram.

## End-to-end acceptance check

1. A consultant signs up, creates a team, adds members, and opens the invite page.
2. Send an invite and verify that the email links to the Otis application host and an opaque `/i/...` URL (never `wavelength.team` or `www.wavelength.team`). Re-send a fresh secure invite to every active participant after deploying this version: legacy bare `/interview/<uuid>` links intentionally no longer grant access.
